	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "FREDOPIE" : "CONFIGURAR CONTROLES", 
	...
	if(numDevices > 0)
		deviceInfo << numDevices << " CONTROLES" << (numDevices > 1 ? "S" : "") << " DETECTADOS";
	else
		deviceInfo << "SEM CONTROLES DETECTADOS";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "PRESSIONE QQ BOTÃO PARA CONFIGURAR", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "F4 PARA SAIR." : "ESC PARA CANCELAR";
