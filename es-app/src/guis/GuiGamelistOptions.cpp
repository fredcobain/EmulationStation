	...
	mMenu(window, "OPÇÕES")
{
	...
	mJumpToLetterList = std::make_shared<LetterList>(mWindow, "IR PARA LETRA", false);
	for(char c = 'A'; c <= 'Z'; c++)
		mJumpToLetterList->add(std::string(1, c), c, c == curChar);

	ComponentListRow row;
	row.addElement(std::make_shared<TextComponent>(mWindow, "IR PARA LETRA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	// edit game metadata
	row.elements.clear();
	row.addElement(std::make_shared<TextComponent>(mWindow, "EDITAR METADATA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
